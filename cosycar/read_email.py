...
class ReadEmail():
    ...
    def fetch_old(self):
        ...
        for email_id in id_list:
            curr_sub = None
            typ, data = mail.fetch(
                str(int(email_id)), 'BODY[HEADER.FIELDS (SUBJECT)]')
            ...
            if found_time:
                ...
                date_to_leave = "{}:{}{}:{}{}".format(now_date, curr_sub[0],
                                                      curr_sub[1], curr_sub[2],
                                                      curr_sub[3])
                date_to_leave = datetime.datetime.strptime(
                    date_to_leave, '%Y:%m:%d:%H:%M')
